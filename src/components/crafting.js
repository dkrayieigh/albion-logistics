...
export function renderCraftingQueue() {
  ...
  craftingQueue.forEach(q => {
    ...
    <td style="display:flex; flex-direction:column; gap:8px;">
      ...
      ${q.artifactName ? `<div><div style="font-size:0.7rem; color:var(--accent-cyan);">${q.artifactName}</div><input type="text" class="format-num queue-art-price" style="width:100%; padding:4px 6px; font-size:0.75rem;" placeholder="單件神器成本" value="${q.artifactPrice ? q.artifactPrice : ''}" data-id="${q.id}"></div>` : ''}
