...
- **Current implementation:** location quantity storage remains legacy `qtyByCity` with literal city/custom location string keys. Read-only adapters, resolver, pure `createCleanInitialState()`, pure new-schema storage codec, injected new-schema storage repository, explicit injected browser Storage backend binding, and explicit browser new-schema repository composition helper exist, but writer/storage paths do not use the future Location Registry model.
...
- **Schema contract status:** Location Registry shape, new root state, `qtyByLocation` inventory shape, clean initialization input/output, storage codec validation/serialization, injected repository load/save semantics, explicit browser Storage backend binding semantics, browser new-schema repository composition semantics, error codes, and backup boundary are defined. Pure initializer, codec, injected repository, browser Storage backend, and browser new-schema repository composition helpers are implemented and covered by regression tests. First-launch confirmation is not implemented.
- **Remaining gaps:** global browser storage acquisition, production bootstrap composition invocation, actual application startup load, first-launch decision/confirmation, current state replacement, writer integration, backup export/import, UI/manual initialization, smoke/release gate.
Current status override for clean initializer implementation:

- **Current implemented:** schema contract exists, pure `createCleanInitialState()` exists, pure `encodeNewSchemaState()` / `decodeNewSchemaState()` exist, pure `createNewSchemaStorageRepository(backend)` exists, pure `createBrowserStorageBackend(storage)` exists, pure `createBrowserNewSchemaRepository(storage)` exists, and these helper groups have regression coverage.
...
- **Isolation boundary:** the helpers remain isolated. `createBrowserStorageBackend(storage)` is an explicit injected binding only, and `createBrowserNewSchemaRepository(storage)` is explicit injected composition only; neither helper is global browser storage acquisition, production bootstrap invocation, startup loading, autosave, production state writing, backup import/export, migration, or UI integration.
- **Remaining gaps:** global browser storage acquisition, production bootstrap composition invocation, actual application startup load, first-launch decision/confirmation, current state replacement, writer integration, backup export/import, UI/manual initialization, smoke/release gate.
