...
- **Current implementation:** location quantity storage remains legacy `qtyByCity` with literal city/custom location string keys. Read-only adapters, resolver, pure `createCleanInitialState()`, pure new-schema storage codec, and injected new-schema storage repository exist, but writer/storage paths do not use the future Location Registry model.
...
- **Schema contract status:** Location Registry shape, new root state, `qtyByLocation` inventory shape, clean initialization input/output, storage codec validation/serialization, injected repository load/save semantics, error codes, and backup boundary are defined. Pure initializer, codec, and injected repository helpers are implemented and covered by regression tests. First-launch confirmation is not implemented.
- **Remaining gaps:** production browser-storage backend binding, actual application startup load, first-launch decision/confirmation, current state replacement, writer integration, backup export/import, UI/manual initialization, smoke/release gate.
Current status override for clean initializer implementation:

- **Current implemented:** schema contract exists, pure `createCleanInitialState()` exists, pure `encodeNewSchemaState()` / `decodeNewSchemaState()` exist, pure `createNewSchemaStorageRepository(backend)` exists, and these helper groups have regression coverage.
...
- **Isolation boundary:** the helpers remain isolated and are not global `localStorage` adapters, startup loaders, autosave, production state writers, backup repositories, migration runners, or UI integration.
- **Remaining gaps:** production browser-storage backend binding, actual application startup load, first-launch decision/confirmation, current state replacement, writer integration, backup export/import, UI/manual initialization, smoke/release gate.
