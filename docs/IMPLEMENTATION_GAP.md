...
- **Current implementation:** location quantity storage remains legacy `qtyByCity` with literal city/custom location string keys. Read-only adapters, resolver, pure `createCleanInitialState()`, and pure new-schema storage codec exist, but writer/storage paths do not use the future Location Registry model.
...
- **Schema contract status:** Location Registry shape, new root state, `qtyByLocation` inventory shape, clean initialization input/output, storage codec validation/serialization, error codes, and backup boundary are defined. Pure initializer and codec helpers are implemented and covered by regression tests. First-launch confirmation is not implemented.
- **Remaining gaps:** injected storage repository contract, actual storage key read/write implementation, startup load behavior, missing/corrupt storage handling, launch confirmation, production ID generator decision/integration, writer integration, new backup export/import, UI/manual initialization flow, smoke/release gate.
Current status override for clean initializer implementation:

- **Current implemented:** schema contract exists, pure `createCleanInitialState()` exists, pure `encodeNewSchemaState()` / `decodeNewSchemaState()` exist, and both helper groups have regression coverage.
- **Covered behavior:** initializer output uses the new root shape, fixed registry, `qtyByLocation`, future canonical `滿日誌` laborer defaults, ordered/unique errors, atomic failure, and input/localStorage immutability. The codec validates schemaVersion 1, assets, fixed/custom Location Registry, `qtyByLocation`, JSON-safe transactions/logs, canonical laborer inventory, legacy-field rejection, malformed JSON, and purity/atomicity.
- **Isolation boundary:** the helpers remain isolated and are not `localStorage` adapters, persisted registries, startup loaders, backup serializers, production writers, migration runners, or UI integration.
- **Remaining gaps:** injected storage repository contract, actual storage key read/write implementation, startup load behavior, missing/corrupt storage handling, first-launch confirmation, production custom ID generator integration, writer integration, backup format implementation, UI/manual initialization, smoke/release gate.
