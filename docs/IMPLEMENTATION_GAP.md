...
- **Current implementation:** location quantity storage remains legacy `qtyByCity` with literal city/custom location string keys. Read-only adapters, resolver, pure `createCleanInitialState()`, pure new-schema storage codec, injected new-schema storage repository, explicit injected browser Storage backend binding, explicit browser new-schema repository composition helper, isolated startup loader/decision helpers, and bidirectional runtime compatibility bridge exist, but writer/storage paths do not use the future Location Registry model.
...
- **Schema contract status:** Location Registry shape, new root state, `qtyByLocation` inventory shape, clean initialization input/output, storage codec validation/serialization, injected repository load/save semantics, explicit browser Storage backend binding semantics, browser new-schema repository composition semantics, startup loader/decision semantics, runtime bridge semantics, error codes, and backup boundary are defined. Pure initializer, codec, injected repository, browser Storage backend, browser new-schema repository composition, startup loader/decision, and runtime bridge helpers are implemented and covered by regression tests. First-launch confirmation is not implemented.
- **Remaining gaps:** global browser storage acquisition, production bootstrap composition invocation, runtime state replacement, canonical save path, writer/storage switch, first-launch UI, backup integration, UI/manual initialization, smoke/release gate.
Current status override for clean initializer implementation:

- **Current implemented:** schema contract exists, pure `createCleanInitialState()` exists, pure `encodeNewSchemaState()` / `decodeNewSchemaState()` exist, pure `createNewSchemaStorageRepository(backend)` exists, pure `createBrowserStorageBackend(storage)` exists, pure `createBrowserNewSchemaRepository(storage)` exists, pure `loadBrowserNewSchemaState(storage)` exists, pure `resolveBrowserNewSchemaStartup(storage)` exists, pure `projectNewSchemaToRuntime(newSchemaState)` exists, pure `projectRuntimeToNewSchema(runtimeState)` exists, and these helper groups have regression coverage.
...
- **Known bridge limitation:** runtime custom location changes cannot yet be safely reverse-saved unless `customLocations` still matches the retained `locationRegistry`.
...
- **Isolation boundary:** the helpers remain isolated. `createBrowserStorageBackend(storage)`, `createBrowserNewSchemaRepository(storage)`, startup helpers, and runtime bridge helpers are not production global browser storage acquisition, production bootstrap invocation, runtime state replacement, canonical save path, writer/storage switch, backup import/export, migration, or UI integration.
- **Remaining gaps:** global browser storage acquisition, production bootstrap composition invocation, runtime state replacement, canonical save path, writer/storage switch, first-launch UI, backup integration, UI/manual initialization, smoke/release gate.
