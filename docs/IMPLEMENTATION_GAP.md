...
- Historical D79 blocker：`Hideout` current mutation was not covered by registry-boundary regression coverage；D80-D81 已補 residual `Hideout` unresolved / `deprecatedLegacyKey` resolver coverage。
- Historical D79 blocker：`LaborerIsland` fixed mapping was not covered by resolver coverage；D80-D81 已補 `LaborerIsland` -> `laborer_island` resolver coverage。
- Writer/storage migration remains blocked；不得寫回 `qtyByLocation`、不得取代 `qtyByCity`、不得建立 Location Registry storage、不得修改 purchase/transport writers、不得移除 legacy fallback。

## D80-D81 Location identity resolver checkpoint

D80-D81 已完成 Location identity mapping contract 與 minimal read-only `resolveLocationIdentity()` implementation。此 checkpoint 只代表 read-only resolver coverage complete，不代表 Location Registry persistence、`qtyByLocation` storage、`customLocations` object migration、writer/storage integration、backup migration 或 migration execution 已開始。

- Current minimal read-only implementation：`src/adapters/locationIdentity.js` 的 `resolveLocationIdentity()`。
- Covered behavior：exact system city mapping、`LaborerIsland` -> `laborer_island`、residual `Hideout` unresolved + `deprecatedLegacyKey`、explicit custom mapping only、unknown unresolved、malformed mapping unresolved、normalized-name conflict detection、no fuzzy matching、input/mapping immutability、system mapping cannot be overridden by custom mapping。
