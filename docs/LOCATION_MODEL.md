...
- `dailyBonusPercent` uses percent points and accepts exactly `0`, `10`, or `20`.
...
### Historical Production-Location Draft - Superseded

The former draft embedded `region` and `regionQuality` directly inside
`state.customLocations`, accepted decimal `dailyBonus` / `hideoutPower`
inputs, and used an LPB formula without the percent-point conversion.

Do not implement that model.

It is superseded by `CRAFTING_DOMAIN_MODEL.md`:

- Location Registry and Production Profile are separate.
- `dailyBonusPercent` accepts exactly `0`, `10`, and `20`.
- `hideoutPowerLevel` represents an integer level, not a decimal LPB value.
- Royal-city and hideout validation follow the canonical facility-specific matrix.
- RRR uses `1 - 1 / (1 + totalLpbPercent / 100)`.
- Region specialization mapping belongs to the canonical crafting-domain contract.
- Production Profile and crafting-event parameters remain target-only and are not current production state.
