...
* **匯入對接規則：** `LABORER_IMPORT` 應直接透過 `${StableId}_${itemLevel}` 組合出目標 `state.inventory` Key，實現無轉換成本的寫入流程。
...
// state.transactions[index]
{
  ...
  location: String,    // 發生地點 (例: "Thetford"，全局校正填 "-")
...
// state.transactions[index]
{
  ...
  type: String,
  item: String,
  itemLevel: String,   // ✅ 正名：取代舊版的 quality
  qty: Number,
  total: Number,
  unitPrice: Number,
  location: String
